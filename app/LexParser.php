...
class LexParser
{
	...
	public function parse($text, $data = array(), $callback = false, $allow_php = false)
	{
		...
		else
		{
			...
			$data = array_merge(LexParser::$data, $data);
		...
		// set $cumulative_noparse to true and use LexParser::inject_noparse($text); immediately
	...
	public function parse_callback_tags($text, $data, $callback)
	{
		...
		while (preg_match($regex, $text, $match, PREG_OFFSET_CAPTURE))
		{
			...
			{
				$cb_data = $data;
				if ( !empty(LexParser::$callback_data))
				{
					$cb_data = array_merge(LexParser::$callback_data, (array) $data);
				}
	...
	public function parse_recursives($text, $orig_text, $callback)
	{
		...
		if (preg_match($this->recursive_regex, $text, $match))
		{
			...
			$tag = $match[0];
			$next_tag = null;
			$children = LexParser::$callback_data[$array_key];
	...
	{
		if (isset(LexParser::$extractions['noparse']))
		{
			foreach (LexParser::$extractions['noparse'] AS $hash => $replacement)
			{
	...
	protected function create_extraction($type, $extraction, $replacement, $text)
	{
		...
		LexParser::$extractions[$type][$hash] = $replacement;
	...
	protected function inject_extractions($text, $type = null)
	{
		if ($type === null)
		{
			foreach (LexParser::$extractions as $type => $extractions)
			{
				foreach ($extractions as $hash => $replacement)
				{
					if (strpos($text, "{$type}_{$hash}") !== false)
					{
						...
						unset(LexParser::$extractions[$type][$hash]);
					}
				}
			}
		}
		...
		{
			if ( ! isset(LexParser::$extractions[$type]))
			{
				return $text;
			}

			foreach (LexParser::$extractions[$type] as $hash => $replacement)
			{
				if (strpos($text, "{$type}_{$hash}") !== false)
				{
					$text = str_replace("{$type}_{$hash}", $replacement, $text);
