...
const findTemplateFiles = (srcDir, globPattern = '**/*.html') => {
  return new Promise((resolve, reject) => {
    ...
    const pattern = path.join(srcDir, globPattern);
...
/**
 * Find all template partial files in a directory
 * @param {string} srcDir - Source directory
 * @param {string} globPattern - Pattern to match files
 * @returns {Promise<string[]>} - Array of matching file paths
 */
const findTemplatePartialFiles = (srcDir, globPattern = '**/*.html') => {
  return new Promise((resolve, reject) => {
    const pattern = path.join(srcDir, 'partials', globPattern);
    glob(pattern, (err, files) => {
      if (err) {
        reject(err);
        return;
